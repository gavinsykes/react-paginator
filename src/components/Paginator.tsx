...
export function Paginator ({
  ...
  const setToIndex: (index: number) => void = (index: number) => {
    if (onPageChange !== undefined) onPageChange(index)
    setCurrentPageIndex(index)
  }
  const setToFirst: () => void = () => { setToIndex(0) }
  const setToPrevious: () => void = () => {
    if (onPageChange !== undefined && currentPageIndex !== 0) onPageChange(currentPageIndex - 1)
    setCurrentPageIndex(prev => prev === 0 ? prev : prev - 1)
  }
  const setToNext: () => void = () => {
    if (onPageChange !== undefined && currentPageIndex !== totalPages - 1) onPageChange(currentPageIndex + 1)
    setCurrentPageIndex(prev => prev === totalPages - 1 ? prev : prev + 1)
  }
  const setToLast: () => void = () => { setToIndex(totalPages - 1) }
  return (
